...
function Navbar() {
  ...
  const nurseryRef = useRef<HTMLSpanElement>(null);
  const [hideNursery, setHideNursery] = useState(false);
  const hideNurseryRef = useRef(false);
  hideNurseryRef.current = hideNursery;
  ...
  useEffect(() => {
    if (!scrolled) {
      if (hideNurseryRef.current) setHideNursery(false);
      return;
    }
    const GAP_HIDE = 6; // hide only when ~0.06" (≈6px) from icon
    const GAP_SHOW = 14; // hysteresis before showing again
    const compute = () => {
      const titleEl = titleRef.current;
      const actionsEl = actionsRef.current;
      const nurseryEl = nurseryRef.current;
      if (!titleEl || !actionsEl || !nurseryEl) return;
      const hidden = hideNurseryRef.current;
      let titleRight: number;
      const actionsLeft = actionsEl.getBoundingClientRect().left;
      if (hidden) {
        const prevDisplay = nurseryEl.style.display;
        const prevVisibility = nurseryEl.style.visibility;
        const prevPosition = nurseryEl.style.position;
        nurseryEl.style.display = "inline";
        nurseryEl.style.visibility = "hidden";
        nurseryEl.style.position = "absolute";
        titleRight = titleEl.getBoundingClientRect().right;
        nurseryEl.style.display = prevDisplay;
        nurseryEl.style.visibility = prevVisibility;
        nurseryEl.style.position = prevPosition;
        const shouldShow = titleRight + GAP_SHOW <= actionsLeft;
        if (shouldShow !== !hidden && shouldShow) setHideNursery(false);
        else if (!shouldShow && hidden !== true) setHideNursery(true);
      } else {
        titleRight = titleEl.getBoundingClientRect().right;
        const shouldHide = titleRight + GAP_HIDE > actionsLeft;
        if (shouldHide !== hidden) setHideNursery(shouldHide);
      }
    };
    compute();
    let raf = 0;
    ...
    const t = window.setTimeout(compute, 320);
    return () => {
      cancelAnimationFrame(raf);
